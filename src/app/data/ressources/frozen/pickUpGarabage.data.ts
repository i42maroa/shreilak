import { ResourceInterface } from "../../interface/resource.interface";

export const WILD_CLEAN:ResourceInterface = {
    id: "1",
    name:"OBJ.WILD_CLEAN.TITLE",
    description:"OBJ.WILD_CLEAN.DESCRIPTION",
    text:"OBJ.WILD_CLEAN.PROCESS",
    images:[
        {
            alt:"pick",
            url:"assets/images/ressources/frozen/dirty-wood.webp"
        }
    ],
    resources:[
        {
            url:"https://view.genially.com/65a82d8f453b6700143981a7/interactive-content-noel-avec-frozen",
            name:"OBJ.WILD_CLEAN.RES.WOOD"
        }
    ]
}
